...
// btnNodeList.forEach((elem) => {
//   elem.addEventListener("click", event => {
//     /* console.log("click!"); */
//     const btnIndex = [...btnNodeList].indexOf(elem.target); //сделали ноде лист массивом  ищем то на что нажали
...
btnNodeList.forEach(elem => {
  elem.addEventListener("click", (event) => {
    const btnIndex = [...btnNodeList].indexOf(event.target);
    const text = texts.find(({ id }) => id === btnIndex + 1).text;
    console.log(text);

    
    const curretTextNode = document.querySelector(".big");
    curretTextNode.innerText = text; // для записи текста 
